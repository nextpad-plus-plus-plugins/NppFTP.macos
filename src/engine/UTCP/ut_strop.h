...
#else
    // macOS / non-UNICODE: TCHAR==char==UTF-8, so char<->char "conversion" is a
    // bounded copy and the code page is irrelevant (kept for signature parity).
    static void cvtcpy(char * dest, size_t size, const char * source, UINT /*codePage*/ = CP_ACP) {
        if (size == 0) return; ::strncpy(dest, source, size - 1); dest[size - 1] = 0;
    }
    static void cvtncpy(char * dest, size_t size, const char * src, size_t count) {
        if (size == 0) return; size_t n = (count < size - 1) ? count : size - 1;
        ::strncpy(dest, src, n); dest[n] = 0;
    }
    static void cvtcat(char * dest, size_t size, const char * source) {
        size_t cur = ::strlen(dest); if (cur + 1 >= size) return;
        ::strncat(dest, source, size - cur - 1);
    }
    static void cvtncat(char * dest, size_t size, const char * src, size_t count) {
        size_t cur = ::strlen(dest); if (cur + 1 >= size) return;
        size_t avail = size - cur - 1; size_t n = (count < avail) ? count : avail;
        ::strncat(dest, src, n);
    }
#endif
